...
public class BST<T extends Comparable<? super T>> {
	...
	class BinaryNode {
		...
		BinaryNode parent; //pointer to parent
	...
	public ArrayList<LinkedList<Integer>> createLinkedList() {
		...
		createLinkedListHelper(array, root, 0);
		return array;
	}

	private void createLinkedListHelper(ArrayList<LinkedList<Integer>> array, BinaryNode sRoot, int level) {
		if (sRoot == null)
			return;
		...
		createLinkedListHelper(array, sRoot.left, level + 1);
		createLinkedListHelper(array, sRoot.right, level + 1);
	}
	...
	private boolean isBSTHelper(BinaryNode sRoot) {
		if (sRoot == null)
			return true;
		if (sRoot.left != null) {
			int compareResult = sRoot.left.element.compareTo(sRoot.element);
			if (compareResult > 0)
				return false;
		}
		if (sRoot.right != null) {
			int compareResult = sRoot.right.element.compareTo(sRoot.element);
			if (compareResult < 0)
				return false;
		}
		return isBSTHelper(sRoot.left) && isBSTHelper(sRoot.right);
	}
	
//	public T nextSuccessor()
//	{
//		
//	}
//	
//	private BinaryNode nextSuccessorHelper(B)
//	{
//		
//	}
}